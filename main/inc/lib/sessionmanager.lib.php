...
class SessionManager
{
    ...
    public static function get_count_admin($where_condition = null)
    {
        ...
        if (!empty($where_condition)) {
            ...
            $where_condition = str_replace(
                    array("AND session_active = '0'  )", " AND (  session_active = '0'  )"), array(') GROUP BY s.name HAVING session_active = 0 ', " GROUP BY s.name HAVING session_active = '0' "), $where_condition
            );
        } else {
            $where_condition = "1 = 1";
        }
    ...
    public static function get_sessions_admin($options)
    {
        ...
        $options['where'] = str_replace(
                array("AND session_active = '0'  )", " AND (  session_active = '0'  )"), array(') GROUP BY s.name HAVING session_active = 0 ', " GROUP BY s.name HAVING session_active = '0' "), $options['where']
        );
...
//            }
    ...
    $userId, $start = null, $limit = null, $getCount = false, $getOnlySessionId = false, $getSql = false
    )
    {
        // Database Table Definitions
        $tbl_session = Database::get_main_table(TABLE_MAIN_SESSION);
    ...
    $status, $userId, $getCount = false, $from = null, $numberItems = null, $column = 1, $direction = 'asc', $keyword = null, $active = null, $lastConnectionDate = null
    )
    {
        $tbl_user = Database::get_main_table(TABLE_MAIN_USER);
        $tbl_session = Database::get_main_table(TABLE_MAIN_SESSION);
        ...
        if (api_is_drh()) {
            if (api_drh_can_access_all_session_content()) {
                $count = self::getAllUsersFromCoursesFromAllSessionFromStatus(
                                'drh_all', api_get_user_id(), true, null, null, null, null, $keyword, $active, $lastConnectionDate
                );
            } else {
                $count = self::getAllUsersFromCoursesFromAllSessionFromStatus(
                                'drh', api_get_user_id(), true, null, null, null, null, $keyword, $active, $lastConnectionDate
                );
            }
        } else {
            if (api_is_platform_admin()) {
                $count = self::getAllUsersFromCoursesFromAllSessionFromStatus(
                                'admin', api_get_user_id(), true, null, null, null, null, $keyword, $active, $lastConnectionDate
                );
            } else {
                $count = self::getAllUsersFromCoursesFromAllSessionFromStatus(
                                'teacher', api_get_user_id(), true, null, null, null, null, $keyword, $active, $lastConnectionDate
                );
            }
        }
        return $count;
