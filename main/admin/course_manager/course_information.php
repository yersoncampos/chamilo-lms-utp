...
api_protect_course_admin_manager($_GET['code']);
...
$sql = "SELECT *, cu.status as course_status FROM $table_course_user cu, $table_user u
        WHERE
            ...
            cu.course_code = '".$course_info['code']."' AND
            cu.relation_type <> ".COURSE_RELATION_TYPE_RRHH." AND
            cu.relation_type <> ".COURSE_RELATION_TYPE_COURSE_ADMIN."
            ";
$res = Database::query($sql);
...
if (Database::num_rows($res) > 0) {
	...
	while ($obj = Database::fetch_object($res)) {
		...
		/*$user[] = '<a href="user_information.php?user_id='.$obj->user_id.'&course='.Security::remove_XSS($course_info['code']).'">'.
...
if (!empty($session_list)) {
    foreach($session_list as &$session)  {
        $session[0] = Display::url($session[0], $url.'admin/course_manager/resume_session.php?course='.$course_info['code'].'&id_session='.$session['id'] );
        unset($session[1]);
    }
