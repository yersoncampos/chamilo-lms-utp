...
if (intval($_POST['formSent']) == 1) {
    ...
    $affected_rows = CourseManager::subscribe_courses_to_course_admin($user_id, $courses_list);
    if ($affected_rows)	{
...
$assigned_courses_to_hrm = CourseManager::get_courses_followed_by_course_admin_manager($user_id);
$assigned_courses_code = array_keys($assigned_courses_to_hrm);
foreach ($assigned_courses_code as &$value) {
	$value = "'".$value."'";
}
...
<tr><td width="45%" align="center">
 <?php echo get_lang('FirstLetterCourse');?> :
     <select name="firstLetterCourse" onchange = "xajax_search_courses(this.value,'multiple')">
      <option value="%">--</option>
      <?php
      echo isset($_POST['firstLetterCourse']) ? Display :: get_alphabet_options($_POST['firstLetterCourse']) : null;
      ?>
     </select>
</td>
<td>&nbsp;</td></tr>
